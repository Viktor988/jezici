...
function showUnitNums(){
  ...
  $("#roomNumbers").empty();
  if(roomNum > 0){
    $("#roomNumbers").append(`<div class="col-md-12" style="padding:5px;"><p>Brojevi soba:</p></div>`).hide().show('fast');
  }
...
function showRoomInfo(){
  ...
  if(roomNum > 0){
    $("#roomDesc").append(`<div class="col-md-12" style="padding:5px;"><p>Informacije o prostorijama:</p></div>`).hide().show('fast');
  }
  for(let i = 1; i <= roomNum; i++){
    ...
    <option value="Bedroom" selected>Spavaća soba</option>
    <option value="Living Area">Dnevna soba</option>
  </select><span>Room Type</span></label></div></div>`).hide().show('fast');
    $("#roomDesc").append(`<div class="col-md-6"><div class="form-group">
    <label class="has-float-label"><input type=number id="bedNum-${i}" style="margin-bottom: 10px;" class="form-control rounded" placeholder="&nbsp" onchange="showBedDesc(${i})"><span>Broj kreveta</span></label></div>
    </div>`).hide().show('fast');
...
function showBedDesc(id){
  ...
  $(bedDesc).empty();
  if(bedNum > 0){
    $(bedDesc).append(`<div class="col-md-12" style="padding:5px;"><p>Informacije o krevetima:</p></div>`).hide().show('fast');
...
$(document).ready(function(){
  ...
  $("#delete_room").on("click", function(){ // Show dialog and delete
    let id = $("#room_settings_active").val();
    let room = rooms_map[id];
    if(confirm(`Da li želite da obrišete jedinicu ${room.name}`)){;
      $.ajax({
        type: 'POST',
        ...
        success: function(rezultat){
          ...
          add_change(`Obrisana jedinica ${room.name}`, sve.data.id); // Add changelog
