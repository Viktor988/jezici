...
$("#form_invoice_new_service").click(function(){
  let invoice_currency = currency;
  if(account_access.articles > 0){
    invoice_currency = "RSD";
  }
  $("#form_invoice_services").append(`
    <div class='flex_around form_invoice_service'>
      <div class='form_invoice_service_name'> <input type='text' class='text_input form_invoice_service_name_input' placeholder='Naziv'> </div>
      <div class='form_invoice_service_amount'> <input type='number' class='number_input form_invoice_service_amount_input' value=1> </div>
      <div class='form_invoice_service_price'> <input type='number' class='number_input form_invoice_service_price_input' value=0> ${invoice_currency} </div>
      <div class='form_invoice_service_tax'> <input type='number' class='number_input form_invoice_service_tax_input' value=0> ${invoice_currency} </div>
      <div class='form_invoice_service_total'> 0.00 ${invoice_currency} </div>
      <div class='form_invoice_service_total_tax'> 0.00 ${invoice_currency} </div>
...
$("#form_invoice_add_article").change(function(){
  ...
  let invoice_currency = "RSD";
  $(this).val(-1).change();

  let tax_rate = article['tax_rate'];
  let service_price = article.price;
  let service_tax = 0;
  if(tax_rate == 3 || tax_rate == 4){
    let tax = tax_rate == 3 ? 20 : 10;
  ...
  $("#form_invoice_services").append(`
    <div class='flex_around form_invoice_service'>
      ...
      <div class='form_invoice_service_name'> <input type='text' class='text_input form_invoice_service_name_input' placeholder='Naziv' value='${article.description}'> </div>
      <div class='form_invoice_service_amount'> <input type='number' class='number_input form_invoice_service_amount_input' value=1> </div>
      <div class='form_invoice_service_price'> <input type='number' class='number_input form_invoice_service_price_input' value=${service_price}> ${invoice_currency} </div>
      <div class='form_invoice_service_tax'> <input type='number' class='number_input form_invoice_service_tax_input' value=${service_tax}> ${invoice_currency} </div>
      <div class='form_invoice_service_total'> 0.00 ${invoice_currency} </div>
      <div class='form_invoice_service_total_tax'> 0.00 ${invoice_currency} </div>
      <div class='form_invoice_service_actions'> <div class='list_action'><img class='list_action_icon delete' title='Obriši'> </div> </div>
    </div>`);
...
$("#new_invoice").click(function(){
  ...
  // Total
  $("#form_invoice_total_price").text(`0.00 ${invoice_currency}`);
  $("#form_invoice_total_tax").text(`0.00 ${invoice_currency}`);
  $("#form_invoice_total_with_tax").text(`0.00 ${invoice_currency}`);
...
function invoice_price_update(){
  ...
  $(".form_invoice_service").each(function(){
    ...
    $(this).find(".form_invoice_service_total").text(service_total_price.toFixed(2) + ` ${invoice_currency}`);
    $(this).find(".form_invoice_service_total_tax").text(service_total_tax.toFixed(2) + ` ${invoice_currency}`);
    total_price += service_total_price;
    total_tax += service_total_tax;
    total_with_tax += (service_total_price + service_total_tax);
  });
  $("#form_invoice_total_price").text(total_price.toFixed(2) + ` ${invoice_currency}`);
  $("#form_invoice_total_tax").text(total_tax.toFixed(2) + ` ${invoice_currency}`);
  $("#form_invoice_total_with_tax").text(total_with_tax.toFixed(2) + ` ${invoice_currency}`);
...
function open_res_invoice(res_id){
  ...
  for(let i=0;i<res.room_data.length;i++){
    ...
    if(account_access.articles == 3){
      article = all_articles_name_map[rooms_map[room.id].shortname].code;
    ...
    let service_tax = 0;
    if(account_access.articles == 3){
      // Finding invoice tax by code
      let lol_articles_list = Object.values(all_articles_map);
      let tax_rate = 1;
      for(let i=0;i<lol_articles_list.length;i++){
        if(lol_articles_list[i]["code"] == article){
            tax_rate = lol_articles_list[i]["tax_rate"];
        }
      }
      if(tax_rate == 3 || tax_rate == 4){
        let tax = tax_rate == 3 ? 20 : 10;
    ...
    else if(properties_map[main_lcode].rooms_tax_included == 1){
  ...
  for(let i=0;i<res.services.length;i++){
    let res_service = res.services[i];
    let service_tax = res_service.price * res_service.tax / 100;
    $("#form_invoice_services").append(`
      <div class='flex_around form_invoice_service'>
        <div class='form_invoice_service_name'> <input type='text' class='text_input form_invoice_service_name_input' placeholder='Unesite naziv' value='${res_service.name}'> </div>
        <div class='form_invoice_service_amount'> <input type='number' class='number_input form_invoice_service_amount_input' value=${res_service.amount}> </div>
        <div class='form_invoice_service_price'> <input type='number' class='number_input form_invoice_service_price_input' value=${res_service.price}> ${invoice_currency} </div>
        <div class='form_invoice_service_tax'> <input type='number' class='number_input form_invoice_service_tax_input' value=${service_tax.toFixed(2)}> ${invoice_currency} </div>
        <div class='form_invoice_service_total'> 0.00 ${invoice_currency} </div>
        <div class='form_invoice_service_total_tax'> 0.00 ${invoice_currency} </div>
        <div class='form_invoice_service_actions'> <div class='list_action'><img class='list_action_icon delete' title='Obriši'> </div> </div>
      </div>`);
...
function open_invoice_form(invoice){
  ...
  for(let i=0;i<invoice.services.length;i++){
    ...
    $("#form_invoice_services").append(`
      <div class='flex_around form_invoice_service'>
        ${code}
        <div class='form_invoice_service_name'> <input type='text' class='text_input form_invoice_service_name_input' placeholder='Naziv' value='${service.name}'> </div>
        ...
        <div class='form_invoice_service_price'> <input type='number' class='number_input form_invoice_service_price_input' value=${service.price}> ${invoice_currency} </div>
        <div class='form_invoice_service_tax'> <input type='number' class='number_input form_invoice_service_tax_input' value=${service.tax}> ${invoice_currency} </div>
        <div class='form_invoice_service_total'> 0.00 ${invoice_currency} </div>
        <div class='form_invoice_service_total_tax'> 0.00 ${invoice_currency} </div>
        <div class='form_invoice_service_actions'> <div class='list_action'><img class='list_action_icon delete' title='Obriši'> </div> </div>
      </div>`);
