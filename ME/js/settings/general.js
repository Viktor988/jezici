...
$(document).ready(function(){
  ...
  $("#property_date_format").change(function(){
    let val = $(this).val();
    update_property("date_format", val);
  });
  $("#property_default_currency").change(function(){
    let val = $(this).val();
    update_property("default_currency", val);
  });
  ...
  $("#property_sync").click(function(){
    ...
    $.ajax({
      ...
      success: function(rezultat){
        ...
        add_change_settings();
        document.location.reload(); // Refresh everything
      },
      error: function(xhr, textStatus, errorThrown){
        $(".button_loader").removeClass("button_loader");
        window.alert("Doslo je do greske. " + xhr.responseText);
