...
function get_login(){
  ...
  $.ajax({
    ...
    success: function(rezultat){
      ...
      if(sve.quicksetup_step != 3){
        window.location.href = "https://quicktest.otasync.me";
      }
...
function display_user_info(data){
  ...
  $(".country_select2").append(`<li data-link="Country" value='--'>Country</option>`);
  for(let i=1;i<country_list.length;i++){
    $(".country_select").append(`<option value='${country_list[i][0]}'>${country_list[i][1]}</option>`);
    $(".country_select2").append(`<li data-link="${country_list[i][1]}" value='${country_list[i][0]}'>${country_list[i][1]}</option>`);
  }
  ...
  // Removing tabs that can't be accessed
  if(account_access.changelog == 0){
    $("#menu_changelog").remove();
    $("#tab_changelog").remove();
  }
  if(account_access.channels == 0){
    $("#channels_section").remove();
  }
  if(account_access.guests == 0){
    $("#nav_guests").remove();
    $("#tab_guests").remove();
  }
  if(account_access.invoices == 0){
    $("#nav_invoices").remove();
    $("#tab_invoices").remove();
  }
  if(account_access.avail == 0){
    $("#rest_work_on option[value='avail']").remove();
    $("#rest_work_on option[value='ota']").remove();
    set_checkbox("cal_filter_avail", 0);
    $("#cal_filter_avail").css("pointer-events", "none");
    properties_map[main_lcode].custom_calendar.avail = 0;
  }
  if(account_access.prices == 0){
    $("#prices_section").remove();
    $("#rest_work_on option[value='price']").remove();
    set_checkbox("cal_filter_price", 0);
    properties_map[main_lcode].custom_calendar.price = 0;
    $("#cal_filter_price").css("pointer-events", "none");
  }
  if(account_access.reservations == 0){
    $("#nav_home").remove();
    $("#nav_reservations").remove();
    $("#main_home").remove();
    $("#main_reservations").remove();
    $("#tab_nav_calendar").remove();
    $("#tab_calendar").remove();
    if(account_access.prices == 0 && account_access.restrictions == 0 && account_access.avail == 0){
      $("#nav_calendar").remove();
      $("#tab_restrictions").remove();
    }

  }
  if(account_access.restrictions == 0){
    $("#restrictions_section").remove();
    $("#rest_work_on option[value='min']").remove();
    $("#rest_work_on option[value='max']").remove();
    $("#rest_work_on option[value='closure']").remove();
    set_checkbox("cal_filter_min", 0);
    properties_map[main_lcode].custom_calendar.min = 0;
    $("#cal_filter_min").css("pointer-events", "none");
    if(account_access.prices == 0){
      $("#settings_nav_plans").remove();
    }
  }
  if(account_access.rooms == 0){
    $("#rooms_section").remove();
    $("#extras_section").remove();
    if(account_access.channels == 0){
      $("#settings_nav_rooms").remove();
    }
  }
  if(account_access.statistics == 0){
    $("#nav_statistics").remove();
    $("#nav_reports").remove();
    $("#main_statistics").remove();
    $("#main_reports").remove();
  }
  if(data.user_status != 1){
    $("#settings_nav_users").remove();
    $("#tab_users").remove();
  }
  if(account_access.reservations == 1){
    $("#cal_new_reservation").remove();
    $("#new_reservation").remove();
  }
  if(account_access.invoice == 1){
    $("#new_invoice").remove();
    $("#form_invoice_confirm").remove();
  }
  if(account_access.price == 1){
    $("#new_price").remove();
  }
  if(account_access.restriction == 1){
    $("#new_restriction").remove();
  }
  if(account_access.rooms == 1){
    $("#new_room").remove();
  }
  if(account_access.channels == 1){
    $("#new_channel").remove();
  }
}
...
function display_property_info(data){
  ...
  $("#property_date_format").val(property.date_format).change();
  $("#property_default_currency").val(property.default_currency);
  currency = property.default_currency;
  $("#wubook_ids").text(property.account + " - " + property.lcode);

  var open_calendar = "";
  $(".calendar_input").datepicker({
      language: "en",
      dateFormat: $("#property_date_format").val(),
