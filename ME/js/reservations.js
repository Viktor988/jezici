...
$("#reservations_export").click(function(){
  let $list = $("#reservations_list");
  let reservations = [];
  ...
  if(reservations.length){
    reservations.join(",");
    $("#reservations_export_key").val(main_key);
    $("#reservations_export_account").val(account_name);
    $("#reservations_export_lcode").val(main_lcode);
    $("#reservations_export_ids").val(reservations);
    $("#reservations_export_form").submit();
  }
});
...
function res_html(loc, res){
  ...
  let date_format = $("#property_date_format").val();
  var period = "";
  if(date_format == "dd-M-yyyy"){
    period = iso_to_eur(res.date_arrival).substring(0,6) + " - " + iso_to_eur(res.date_departure).substring(0,6);
  }
  else if(date_format == "mm/dd/yyyy"){
     period = iso_to_eur(res.date_arrival).substring(0,5) + " - " + iso_to_eur(res.date_departure).substring(0,5);
  }
  else if(date_format == "yyyy-mm-dd"){
    period = iso_to_eur(res.date_arrival).substring(5,10) + " - " + iso_to_eur(res.date_departure).substring(5,10);
  }
  else { // "dd/mm/yyyy"
    period = iso_to_eur(res.date_arrival).substring(0,5) + " - " + iso_to_eur(res.date_departure).substring(0,5);
  }
  ...
  if(account_access.reservations == 3 || (account_access.reservation == 2 && res.created_by == user_id)){
    res_edit = `<div class='list_action'><img class='list_action_icon edit' title='Izmjeni'> </div>`;
    res_delete = `<div class='list_action'><img class='list_action_icon delete' title='Obriši'> </div>`;
  }
  if(account_access.invoices >= 2){
    res_invoice = `<div class='list_action'><img class='list_action_icon invoice' title='Račun'> </div>`;
  }
