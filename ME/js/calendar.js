  ...
  $(document).on("mouseup", function(e){
    ...
    if(click_vals.dfrom !== "" && cmp_dates(click_vals.dfrom, click_vals.dto) <= 0){ // If a period is selected
      if(click_vals.field == "room"){ // Opens reservation form for rooms, assumes that the selected room is actualy free
        ...
        // Remove selected range from calendar
        ...
        $('#forms_dates').daterangepicker({
          ...
          "startDate": new Date(click_vals.dfrom),
          "endDate": new Date(click_vals.dto),
          "locale": {
            format: 'YYYY-MM-DD'
          }});
        $('#forms_dates').click();
        ...
        $.ajax({
          ...
          success: function(rezultat){
            ...
            // Add first room
            $(`.form_res_rooms li[value='${click_vals.room}']`).click();
            $(`.form_res_room_numbers li[value='${click_vals.room_number}']`).click();
...
var send_price = function(dfrom, dto, pid, values, rooms, variation_type)
{
  ...
  $.ajax({
    ...
    success: function(rezultat){
      ...
      add_change("Izmjenjena cijena", sve.data.id);
      click_to_hide()
      get_calendar();
...
var send_price_virtual = function(value, variation, pid)
{
  ...
  $.ajax({
    ...
    success: function(rezultat){
      ...
      add_change("Izmjenjene cijene", sve.data.id);
      click_to_hide()
      get_calendar();
    },
    error: function(xhr, textStatus, errorThrown){
