...
function makeRequest($fName, $fArgs)
{
  ...
  if ($response && xmlrpc_is_fault($response)) {
    fatal_error("Request failed", 500);
  }
  else {
      if($response[0] !== 0)
        fatal_error("Error: " . $response[1] . " ($fName)" , 200);
      return $response[1];
  }
}
...
function saveImageBase64($base64_string, $name) {
  $data = explode(',', $base64_string);
  $fileExtention = explode("image/", $data[0]);
  $fileExtention = explode(";", $fileExtention[1]);
  ...
  fclose($file);
  $url = "https://admin.otasync.me/images/" . $fileName;
  return $url;
}
function saveImageBase642($image, $name)
{
  $location = "/images/";
  try {
    // check INI error
...
function insertWubookReservation($lcode, $account, $res)
{
  ...
  if($old_reservation && $old_reservation["status"] ==  1 && $res["status"] == 5){ // Deleted reservation
     ...
     $sql = "UPDATE reservations_$lcode SET
      ...
      if(sizeof($rooms_map) == 0) // If no rooms are found it means that the reservation is from a deleted room.
        return;
...
function sendGuestEmail($reservation_id, $lcode, $account, $konekcija)
{
  ...
  if($email_info["before_active"] != 0){
      if(dateDiff($reservation["date_received"], $reservation["date_arrival"]) <= $email_info["before_active"]){
        $subject = $email_info["before_subject"];
        $message = $email_info["before_text"];
        $message = str_replace( "_NAME_" , $reservation["customer_name"] , $message);
        $message = str_replace( "_SURNAME_" , $reservation["customer_surname"] , $message);
        $message = str_replace( "_DFROM_" , ymdToDmy($reservation["date_arrival"]) , $message);
        $message = str_replace( "_DTO_" , ymdToDmy($reservation["date_departure"]) , $message);
