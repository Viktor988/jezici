...
function old_fixReservation($row, $channel_logos, $room_names, $room_shortnames, $lcode, $konekcija)
{
  if($row["status"] == 5)
    $row["status"] = 0;
  $row["services"] = json_decode($row["services"]);
  ...
  $adults = [];
  $children = [];
  $tresholdDate = strtotime("-12 years");
  $guest_ids = $row["guest_ids"];
  $sql = "SELECT * FROM guests_$lcode WHERE id IN ($guest_ids)";
  $rezultat = mysqli_query($konekcija, $sql);
  while($red = mysqli_fetch_assoc($rezultat)){
    if(strtotime($red["date_of_birth"]) < $tresholdDate){
      array_push($adults, $red);
    }
    else {
      array_push($children, $red);
    }
  }
  $row["adult_guests"] = $adults;
  $row["children_guests"] = $children;
  // Adding channel logo
...
function old_fixReservation_not_used($row, $channel_logos, $room_names, $room_shortnames)
{
  if($row["status"] == 5)
    $row["status"] = 0;
  $row["services"] = json_decode($row["services"]);
...
if($action == "events")
{
  ...
  if(in_array("arrival", $status) || (in_array("arrivals", $status))){
    ...
    $rezultat = mysqli_query($konekcija, $sql);
    while($red = mysqli_fetch_assoc($rezultat)){
      $res = old_fixReservation($red, $channel_logos, $room_names, $room_shortnames, $lcode, $konekcija);
...
if($action == "search")
{
  ...
  $rezultat = mysqli_query ($konekcija, $sql);
  while($red = mysqli_fetch_assoc($rezultat)){
    array_push($reservations, old_fixReservation($red, $channel_logos, $room_names, $room_shortnames, $lcode, $konekcija));
  }
...
if($action == "calendarGantt")
{
    ...
    while($red = mysqli_fetch_assoc($rezultat)){
        ...
        $res_data = old_fixReservation($red, $channel_logos, $room_names, $room_shortnames, $lcode, $konekcija);
        for($i=0;$i<sizeof($res_rooms);$i++){
    ...
    if(checkPostExists("price_id") && checkPostExists("restriction_id")){
      $price_id = checkPost("price_id");
      $restriction_id = checkPost("restriction_id");
      $avail = plansAvailValues($lcode, $dfrom, $dto, $konekcija);
      $prices = plansPriceValues($lcode, $dfrom, $dto, $price_id, $konekcija);
      ...
      $ret_val["additional_data"] = $values;
    }
}
