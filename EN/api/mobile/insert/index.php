...
if($action == "reservation")
{
  ...
  if(!(checkPostExists("id"))) // New
  {
    ...
    // Remember data
    $sql = "SELECT * FROM reservations_$lcode WHERE reservation_code = '$change_id'";
    $rezultat = mysqli_query($konekcija, $sql);
    if($rezultat)
      $old_data = mysqli_fetch_assoc($rezultat);
    else
...
if($action == "payment")
{
  ...
  $date = checkPost("date");
  $payment_gateway_fee = [];
  $payment_gateway_fee["payment_type"] = $payment_type;
  $payment_gateway_fee["date"] = $date;
  $payment_gateway_fee["value"] = $value;
  $payment_gateway_fee["type"] = $type;
  $payment_gateway_fee = json_encode($payment_gateway_fee);
  $sql = "UPDATE reservations_$lcode SET payment_gateway_fee = '$payment_gateway_fee' WHERE reservation_code = '$id'";
  $rezultat = mysqli_query($konekcija, $sql);
  $ret_val["data"] = [];
  $ret_val["data"]["new_data"] = [];
  $ret_val["data"]["new_data"]["payment_gateway_fee"] = json_decode($payment_gateway_fee);
}
...
if($action != ""){
  $data_type = $action;
  $ch_action = "edit";
  if($action=="reservation" && !(checkPostExists("id")))
    $ch_action = "insert";
  $user_name = $user["name"];
