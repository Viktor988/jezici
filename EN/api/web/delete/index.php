...
if($action == "reservation")
{
  ...
  if($user["reservations"] < 2 || ($user["reservations"] == 2 && $old_data["created_by"] != $user["id"])){
...
if($action == "guest")
{
  ...
  if($user["guests"] < 2 || ($user["guests"] == 2 && $old_data["created_by"] != $user["id"])){
      fatal_error("No access", 200);
  }
...
if($action == "invoice")
{
  ...
  if($user["invoices"] < 2 || ($user["invoices"] == 2 && $old_data["created_by"] != $user["id"])){
      fatal_error("No access", 200);
  }
...
if($action == "pricingPlan")
{
  ...
  if($user["prices"] < 2 || ($user["prices"] == 2 && $old_data["created_by"] != $user["id"])){
      fatal_error("No access", 200);
  }
...
if($action == "restrictionPlan")
{
  ...
  if($user["restrictions"] < 2 || ($user["restrictions"] == 2 && $old_data["created_by"] != $user["id"])){
      fatal_error("No access", 200);
  }
...
if($action == "room")
{
  ...
  if($user["rooms"] < 2 || ($user["rooms"] == 2 && $old_data["created_by"] != $user["id"])){
      fatal_error("No access", 200);
  }
  ...
  if(!$rezultat)
    ...
    // Updating calendar for deleted room
    ...
    for($i=0;$i<sizeof($old_real_rooms);$i++){
      if($old_real_rooms[$i] != $id) // Push from old to new only if it's not deleted
        array_push($new_real_rooms, $old_real_rooms[$i]);
    }
    for($i=0;$i<sizeof($old_single_rooms);$i++){
      $old_room_id_start = explode("_", $old_single_rooms[$i])[0];
      if($old_single_rooms[$i] != $id) // Push from old to new only if it's not deleted
...
if($action == "channel")
{
  ...
  if($user["channels"] < 2 || ($user["channels"] == 2 && $old_data["created_by"] != $user["id"])){
      fatal_error("No access", 200);
  }
