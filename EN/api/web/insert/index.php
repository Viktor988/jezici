...
{
  if($user["reservations"] < 2){
    fatal_error("No access", 200);
  }
  // Post data
  ...
  if($resp[0] == 0){
    ...
    for($i=0;$i<sizeof($guests);$i++){
      ...
      if($id == ""){ // New guest
        ...
        $sql = "INSERT INTO guests_$lcode (name, surname, email, phone, country_of_residence, place_of_residence, address, zip, country_of_birth, date_of_birth, gender, host_again, note, total_arrivals, total_nights, total_paid, registration_data, id_number, id_type, created_by) VALUES (
          '$name',
          '$surname',
          '$email',
          '$phone',
...
if($action == "guest")
{
  ...
  $id_type = "--";
  $id_number = "";
  if(checkPostExists("id_type"))
    $id_type = checkPost("id_type");
  if(checkPostExists("id_number"))
    $id_number = checkPost("id_number");
...
if($action == "room")
{
  ...
  $images = json_decode(checkPost("images"));
  $image_links = [];
  for($i=0;$i<sizeof($images);$i++){
    array_push($image_links, saveImageBase64($images[$i], $lcode . "_" . "$id" . "_" . $i));
  }
  ...
  array_push($custom_cal["room_types"], $id);
  for($i=0;$i<$availability;$i++){
    array_push($custom_cal["single_rooms"], $id . "_$i");
  }
...
if($action == "initialRooms")
{
  ...
  for($i=0;$i<sizeof($rooms);$i++){
    // Wubook delete
    makeUncheckedRequest("del_room", array($userToken, $lcode, $id));
    // DB delete
    $sql = "DELETE FROM rooms_$lcode WHERE id = '$id'";
    $rezultat = mysqli_query($konekcija, $sql);
  }
  $new_data = [];
  ...
  for($i=0;$i<sizeof($rooms);$i++){
    ...
    for($j=0;$j<sizeof($images);$j++){
      array_push($image_links, saveImageBase64($images[$i], $lcode . "_" . time() . "_" . $i));
    }
    $images = json_encode($image_links);
    $room_status = [];
    ...
    if($rezultat){
      ...
      $new_id = makeRequest("new_room", array($userToken, $lcode, 0, $name, $occupancy, $price, $availability, $shortname, 'nb'));
      $sql = "UPDATE rooms_$lcode SET id = '$new_id' WHERE id = '$id'";
      mysqli_query($konekcija, $sql);
      array_push($cal_real_rooms, $new_id);
      for($j=1;$j<=$availability;$j++){
        array_push($room_status, "clean");
        array_push($cal_single_rooms, $new_id . "_" . ($j - 1));
      }
    }
    else {
     fatal_error("Database error", 500); // Server failed
  ...
  $sql = "UPDATE all_users SET quicksetup_step = 3 WHERE id = '$user_id'";
  mysqli_query($konekcija, $sql);
...
if($action == "user")
{
  ...
  $sql = "SELECT COUNT(*) as cnt FROM all_users WHERE email = '$user_email'";
  $rezultat = mysqli_query($konekcija, $sql);
  $red = mysqli_fetch_assoc($rezultat);
  $cnt = $red["cnt"];
  if($cnt > 0)
    fatal_error("Email already in use", 200);
  ...
  invoice_margin, invoice_issued, invoice_delivery, room_count, ctypes, booking, booking_percentage, expedia, airbnb, private, agency, split, pricing_plan, quicksetup_step)
  VALUES
  (
    '$user_email',
    '$user_key',
  ...
  if($rezultat){
    ...
    $message = "For registration on admin.otasync.me for the account $account please click this link: $link";
