...
if($action == "room")
{
  ...
  if(!$rezultat)
    ...
    $userToken = makeRequest("acquire_token", array($account, "davincijevkod966", "753fa793e9adb95321b061f05e29a78327645c05e097e376"));
    $resp = makeUncheckedRequest("mod_room", array($userToken, $lcode, $id, $name, $occupancy, $price, $availability, $shortname, 'nb'));
    makeReleaseRequest("release_token", array($userToken));
  }
}
...
if($action == "restrictionCompact")
{
  ...
  if(checkPostExists("name")){
    $name = checkPost("name");
    $sql = "UPDATE restrictions_$lcode SET name = '$name' WHERE id = '$id'";
    $rezultat = mysqli_query($konekcija, $sql);
  }
