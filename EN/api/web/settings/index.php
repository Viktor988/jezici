...
if($action == "sync")
{
  ...
  // Rooms
  $rooms = makeRequest("fetch_rooms", array($userToken, $lcode, 0));
  // Arrays of rooms used for calendar settings
  $cal_real_rooms = [];
  $cal_single_rooms = [];
  // Array of all current wubook rooms
  $new_rooms = [];
  for($i=0;$i<sizeof($rooms);$i++){
    array_push($new_rooms, $rooms[$i]["id"]);
    $id = $rooms[$i]["id"];
    $name = $rooms[$i]["name"];
    $shortname = $rooms[$i]["shortname"];
    $occupancy = $rooms[$i]["occupancy"];
    ...
    // If room already exists, update data
  ...
  if(sizeof($new_rooms)){
    $old_rooms = [];
    // Get list of rooms that aren't available on wubook
    $sql = "SELECT id FROM rooms_$lcode";
    $rezultat = mysqli_query($konekcija, $sql);
    while($red = mysqli_fetch_assoc($rezultat)){
      if(!(in_array($red["id"], $new_rooms)))
      array_push($old_rooms, $red["id"]);
    }
    for($a=0;$a<sizeof($old_rooms);$a++){
      $old_room_id = $old_rooms[$a];
      $sql = "DELETE FROM rooms_$lcode WHERE id = '$old_room_id'";
      mysqli_query($konekcija, $sql);
    }
  }
  // Updating calendar for new rooms
  $sql = "SELECT custom_calendar FROM all_properties WHERE lcode = '$lcode'";
  $rezultat = mysqli_query($konekcija, $sql);
  $custom_cal = mysqli_fetch_assoc($rezultat);
  $custom_cal = json_decode($custom_cal["custom_calendar"], true);
  $old_real_rooms = $custom_cal["room_types"];
  $old_single_rooms = $custom_cal["single_rooms"];
  $all_real_rooms = $cal_real_rooms;
  $all_single_rooms = $cal_single_rooms;
  $new_real_rooms = [];
  $new_single_rooms = [];
  for($i=0;$i<sizeof($old_real_rooms);$i++){
    if(in_array($old_real_rooms[$i], $all_real_rooms)) // Push from old to new only if it still exists
      array_push($new_real_rooms, $old_real_rooms[$i]);
  }
  for($i=0;$i<sizeof($old_single_rooms);$i++){
    if(in_array($old_single_rooms[$i], $all_single_rooms)) // Push from old to new only if it still exists
      array_push($new_single_rooms, $old_single_rooms[$i]);
  }
  $custom_cal["room_types"] = $new_real_rooms;
  $custom_cal["single_rooms"] = $new_single_rooms;
  $custom_cal = json_encode($custom_cal);
  $sql = "UPDATE all_properties SET custom_calendar = '$custom_cal' WHERE lcode = '$lcode'";
  mysqli_query($konekcija, $sql);

  // Pricing plans
  $pricing_plans = makeRequest("get_pricing_plans", array($userToken, $lcode));
  $default_price = "";
  // Array of all current wubook prices
  $new_prices = [];
  for($i=0;$i<sizeof($pricing_plans);$i++){
    array_push($new_prices, $pricing_plans[$i]["id"]);
    $plan_id = $pricing_plans[$i]["id"];
    $plan_name = $pricing_plans[$i]["name"];
    $plan_type = "daily";
    $plan_variation = "";
    ...
    name = '$plan_name',
    type = '$plan_type',
    variation = '$plan_variation',
    variation_type = '$plan_variation_type',
    vpid = '$plan_vpid'
    WHERE id = '$plan_id'";
    mysqli_query($konekcija, $sql);
    // Insert new plan
  ...
  // Deleting old prices
  if(sizeof($new_prices)){
    $old_prices = [];
    // Get list of prices that aren't available on wubook
    $sql = "SELECT id FROM prices_$lcode";
    $rezultat = mysqli_query($konekcija, $sql);
    while($red = mysqli_fetch_assoc($rezultat)){
      if(!(in_array($red["id"], $new_prices)))
      array_push($old_prices, $red["id"]);
    }
    for($a=0;$a<sizeof($old_prices);$a++){
      $old_price_id = $old_prices[$a];
      $sql = "DELETE FROM prices_$lcode WHERE id = '$old_price_id'";
      mysqli_query($konekcija, $sql);
    }
  }
  ...
  // Restriction plans
  $restriction_plans = makeRequest("rplan_rplans", array($userToken, $lcode));
  // Array of all current wubook restrictions
  $new_restrictions = [];
  for($i=0;$i<sizeof($restriction_plans);$i++){
    array_push($new_restrictions, $restriction_plans[$i]["id"]);
    $plan_id = $restriction_plans[$i]["id"];
    $plan_name = $restriction_plans[$i]["name"];
    $plan_type = "daily";
    $plan_rules = "{}";
  ...
  // Deleting old restrictions
  if(sizeof($new_restrictions)){
    $old_restrictions = [];
    // Get list of restrictions that aren't available on wubook
    $sql = "SELECT id FROM restrictions_$lcode";
    $rezultat = mysqli_query($konekcija, $sql);
    while($red = mysqli_fetch_assoc($rezultat)){
      if(!(in_array($red["id"], $new_restrictions)))
      array_push($old_restrictions, $red["id"]);
    }
    for($a=0;$a<sizeof($old_restrictions);$a++){
      $old_restriction_id = $old_restrictions[$a];
      $sql = "DELETE FROM restrictions_$lcode WHERE id = '$old_restriction_id'";
      mysqli_query($konekcija, $sql);
    }
  }
  ...
  $new_channels = [];
  for($i=0;$i<sizeof($channels);$i++){
    array_push($new_channels, $channels[$i]["id"]);
    $id = $channels[$i]["id"];
    $ctype = $channels[$i]["ctype"];
    $hotel_id = $channels[$i]["channel_hid"];
    $sql = "SELECT name, logo, commission
  ...
  if(sizeof($new_channels)){
    $old_channels = [];
    // Get list of channels that aren't available on wubook and aren't custom
    $sql = "SELECT id FROM channels_$lcode WHERE ctype > 0";
    $rezultat = mysqli_query($konekcija, $sql);
    while($red = mysqli_fetch_assoc($rezultat)){
      if(!(in_array($red["id"], $new_channels)))
      array_push($old_channels, $red["id"]);
    }
    for($a=0;$a<sizeof($old_channels);$a++){
      $old_channel_id = $old_channels[$a];
      $sql = "DELETE FROM channels_$lcode WHERE id = '$old_channel_id'";
      mysqli_query($konekcija, $sql);
    }
  }
