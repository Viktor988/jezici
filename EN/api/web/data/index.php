...
if($action == "reservations")
{
  if($user["reservations"] > 0){
    ...
    $ret_val["min_price"] = 0;
    $ret_val["max_price"] = 0;
    $ret_val["min_nights"] = 0;
    $ret_val["max_nights"] = 0;
    $ret_val["page"] = 0;
    $ret_val["total_pages_number"] = 0;
  }
}
...
if($action == "statistics")
{
  if($user["statistics"] > 0){
    ...
    $statistics["rooms"] = [];
    $statistics["channels"] = [];
    $statistics["channels_percentage"] = [];
    $statistics["countries_percentage"] = [];
    $statistics["bookwindow"] = [];
    $ret_val["data"] = [];
  }
}
...
if($action == "groupResRooms") // Used for free rooms and prices of group reservations
{
  ...
  foreach($avail as $room => $dates){
    ...
    // Exclude closed
    $sql = "SELECT MAX(closed_$room) AS closed FROM restrictions_values_$lcode WHERE id = '1' AND restriction_date >= '$dfrom' AND restriction_date < '$dto'";
    $rezultat = mysqli_query($konekcija, $sql);
...
if($action == "dailyReport")
{
  if($user["statistics"] > 0){
    ...
    $room_names = [];
    $all_room_numbers = [];
    $sql = "SELECT id, name, room_numbers FROM rooms_$lcode WHERE parent_room = 0";
    $rezultat = mysqli_query($konekcija, $sql);
    while($red = mysqli_fetch_assoc($rezultat))
    {
...
if($action == "occupancyReport")
{
  if($user["statistics"] > 0){
    ...
    $rooms = [];
    $rooms_cmp = [];
    $sql = "SELECT id, name, price, availability FROM rooms_$lcode WHERE id IN($room_ids) AND parent_room = '0'";
    $rezultat = mysqli_query($konekcija, $sql);
    while($red = mysqli_fetch_assoc($rezultat)){
        $rooms[$red["id"]] = $red;
    ...
    // Fetch reservations
    $sql = "SELECT real_rooms, room_data, nights, date_arrival FROM reservations_$lcode WHERE date_arrival <= '$date' AND date_departure > '$date' AND status = 1";
    $rezultat = mysqli_query($konekcija, $sql);
    while($red = mysqli_fetch_assoc($rezultat)){
      $red = fixReservation($red, $channel_data, $konekcija, $lcode);
      ...
      for($i=0;$i<sizeof($res_rooms);$i++) // Individual rooms
      {
        ...
        if(isset($rooms[$res_room->parent_id])){
          $rooms[$res_room->parent_id]["count"] += $res_room->count;
          $rooms[$res_room->parent_id]["total_price"] += ($res_room->count * $res_room->price);
        }
      }
    }
    ...
    {
      // Fetch reservations
      $sql = "SELECT real_rooms, room_data, nights, date_arrival FROM reservations_$lcode WHERE date_arrival <= '$cmp' AND date_departure > '$cmp' AND status = 1";
      $rezultat = mysqli_query($konekcija, $sql);
      while($red = mysqli_fetch_assoc($rezultat))
      {
        $red = fixReservation($red, $channel_data, $konekcija, $lcode);
        ...
        for($i=0;$i<sizeof($res_rooms);$i++) // Individual rooms
        {
          $res_room = $res_rooms[$i];
          if(isset($rooms[$res_room->parent_id])){
            $rooms_cmp[$res_room->parent_id]["count"] += $res_room->count;
            $rooms_cmp[$res_room->parent_id]["total_price"] += ($res_room->count * $res_room->price);
          }
        }
      }
